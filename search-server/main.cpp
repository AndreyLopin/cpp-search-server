...
public:
    ...
    [[nodiscard]] bool AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        vector<string> words;
        if(document_id < 0 || documents_.count(document_id) == 1 || !SplitIntoWordsNoStop(document, words)) {
            return false;
        }
    ...
    optional<vector<Document>> FindTopDocuments(const string& raw_query, DocumentStatus status) const {
        if(const auto result = FindTopDocuments(
    ...
    optional<vector<Document>> FindTopDocuments(const string& raw_query) const {
        if(const auto result = FindTopDocuments(raw_query, DocumentStatus::ACTUAL); result.has_value()) {
            return result;
        } else {
            return nullopt;
        }
